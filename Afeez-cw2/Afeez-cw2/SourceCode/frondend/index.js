document.addEventListener("DOMContentLoaded", function() {
    let activeCoin = "ADA"; // Initial active coin
    ...
    function updateCharts() {
        ...
        if (coinData[activeCoin]) {
            ...
            updateSentimentChart(positivePercentage, negativePercentage, neutralPercentage);
        }
    }
    ...
    function calculatePercentage(sentiment) {
        if (coinData[activeCoin]?.sentiment) {
            ...
            if (sentiment === "positive") {
                sentimentCount = coinData[activeCoin].sentiment.y.filter(item => item > 0).length;
    ...
    function updateTimeseriesChart() {
        ...
        Plotly.newPlot(timeseriesPlot, [
            {
                x: coinData[activeCoin].real?.x.map(item => new Date(item)),
                y: coinData[activeCoin].real?.y,
                mode: 'line',
                type: "scatter",
                marker: { color: 'black' },
                name: "Prices"
    ...
    function updateSentimentChart(positivePercentage, negativePercentage, neutralPercentage) {
        const sentimentChartDiv = document.getElementById("sentimentChart");

        sentimentChartDiv.innerHTML = "";

        Plotly.newPlot(sentimentChartDiv, [
            {
    ...
    // WebSocket setup
    connection = new WebSocket("wss://8o3klxybef.execute-api.us-east-1.amazonaws.com/prod");
